namespace Lidgren.Network
{
    public partial class NetConnection
    {
        ...
        {
            get { return (float) m_averageRoundtripTime; }
        }
        ...
        {
            get { return (float) m_remoteTimeOffset; }
        }
        ...
        internal void InitializePing()
        {
            ...
            m_timeoutDeadline = now + (m_peerConfiguration.m_connectionTimeout*2.0);
        ...
        internal void ReceivedPong(double now, int pongNumber, float remoteSendTime)
        {
            ...
            if (m_averageRoundtripTime < 0)
            {
                ...
                m_peer.LogDebug("Initiated average roundtrip time to " + NetTime.ToReadable(m_averageRoundtripTime) +
                                " Remote time is: " + (now + diff));
            }
            else
            {
                m_averageRoundtripTime = (m_averageRoundtripTime*0.7) + (rtt*0.3);

                m_remoteTimeOffset = ((m_remoteTimeOffset*(m_sentPingNumber - 1)) + diff)/m_sentPingNumber;
                m_peer.LogVerbose("Updated average roundtrip time to " + NetTime.ToReadable(m_averageRoundtripTime) +
                                  ", remote time to " + (now + m_remoteTimeOffset) + " (ie. diff " + m_remoteTimeOffset +
                                  ")");
            }
