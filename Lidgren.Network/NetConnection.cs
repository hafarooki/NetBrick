...
namespace Lidgren.Network
{
    ...
    public partial class NetConnection
    {
        ...
        /// <summary>
        ///     Change the internal endpoint to this new one. Used when, during handshake, a switch in port is detected (due to
        ///     NAT)
        /// </summary>
        ...
        internal void Heartbeat(double now, uint frameCounter)
        {
            ...
            if ((frameCounter%m_messageCoalesceFrames) == 0) // coalesce a few frames
            {
                ...
                while (m_queuedOutgoingAcks.Count > 0)
                {
                    ...
                    if (m_queuedOutgoingAcks.Count > 0)
                    {
                        ...
                        m_peer.SendPacket(m_sendBufferWritePtr, m_remoteEndPoint, m_sendBufferNumMessages,
                            out connectionReset);
            ...
            {
                for (var i = m_sendChannels.Length - 1; i >= 0; i--)
                    ...
                    if (channel != null)
                    {
                        ...
                        if (channel.QueuedSendsCount > 0)
                            m_peer.m_needFlushSendQueue = true;
                                // failed to send all queued sends; likely a full window - need to try again
                    }
                    NetException.Assert(m_sendBufferWritePtr < 1 || m_sendBufferNumMessages > 0);
                }
            }
        ...
        internal NetSendResult EnqueueMessage(NetOutgoingMessage msg, NetDeliveryMethod method, int sequenceChannel)
        {
            ...
            if ((method != NetDeliveryMethod.Unreliable && method != NetDeliveryMethod.UnreliableSequenced) &&
                msg.GetEncodedSize() > m_currentMTU)
        ...
        internal void ReceivedLibraryMessage(NetMessageType tp, int ptr, int payloadLength)
        {
            ...
            switch (tp)
            {
                ...
                case NetMessageType.LibraryError:
                    m_peer.ThrowOrLog(
                        "LibraryError received by ReceivedLibraryMessage; this usually indicates a malformed message");
                    break;
        ...
        /// </summary>
        public void GetSendQueueInfo(NetDeliveryMethod method, int sequenceChannel, out int windowSize,
