    ...
    internal sealed class NetReliableSenderChannel : NetSenderChannelBase
    {
        ...
        internal override int GetAllowedSends()
        {
            var retval = m_windowSize -
                         ((m_sendStart + NetConstants.NumSequenceNumbers) - m_windowStart)%
                         NetConstants.NumSequenceNumbers;
        ...
        internal override NetSendResult Enqueue(NetOutgoingMessage message)
        {
            ...
            m_connection.m_peer.m_needFlushSendQueue = true;
                // a race condition to this variable will simply result in a single superflous call to FlushSendQueue()
                ...
                if (t > 0 && (now - t) > m_resendDelay)
                {
					...
					*/
                    ...
                    Interlocked.Increment(ref om.m_recyclingCount);
                        // increment this since it's being decremented in QueueSendMessage
        ...
        private void DestoreMessage(double now, int storeIndex, out bool resetTimeout)
        {
            ...
            if (storedMessage == null)
                throw new NetException("m_storedMessages[" + storeIndex + "].Message is null; sent " +
                                       m_storedMessages[storeIndex].NumSent + " times, last time " +
                                       (NetTime.Now - m_storedMessages[storeIndex].LastSent) + " seconds ago");
#else
			if (storedMessage != null)
			{
#endif
                ...
                else
                {
                    ...
                    if (m_storedMessages[slot].NumSent == 1)
                    {
                        ...
                        else
                        {
                            ...
                            Interlocked.Increment(ref rmsg.m_recyclingCount);
                                // increment this since it's being decremented in QueueSendMessage
